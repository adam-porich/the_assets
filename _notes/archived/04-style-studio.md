...
- [x] Open Style Studio from the active-style header chip, not from the primary
  Sources → Cards path.
- [x] Lead with the active version, a concise description, generation reference
...
- [x] Provide one **Edit as new version** action. Editing a locked version creates
  or resumes a separate draft and never changes production behavior.
- [x] Keep version history and raw provenance collapsed. Make the active version
  and current draft impossible to confuse.

## Draft editing

- [x] Put one plain-language **What should change?** field first. Preserve the
  structured generation direction as effective fields rather than concatenating
  an untraceable prompt.
- [x] Provide an ordered **Generation references** strip with add, replace,
  remove, and reorder actions. Show the model's exact reference limit including
  the identity source.
- [x] Show target examples in a separate read-only region with no control that
  can move them into the generation stack.
- [x] Put effective controls in three collapsed advanced groups:
  **Generation** (model, quality, direction, avoid), **Amiga processing**
  ...
  and assembly values).
- [x] Render controls from the driver's editor descriptors where practical and
  show validation beside the field. Do not expose a raw JSON editor as the only
  way to make a normal style adjustment.
- [x] Resolve model capabilities when execution mode/model changes and expose
...
- [x] Show the normalized style checksum and changed sections in a compact draft
  summary, not as the main editing interface.

## Calibration cohort

- [x] Let the user pin up to three representative workspace sources as the
...
- [x] Keep the cohort stable across draft trials until the user changes it so
  comparisons remain meaningful.
- [x] Require at least one source and a valid draft. Show exact paid calls before
  **Test complete style** starts.
- [x] A style trial must generate one master per calibration source and
  immediately apply draft framing, Amiga processing, and card assembly. There is
  no master-only success state.
- [x] Reuse the production worker, renderer registry, provenance, polling,
  partial failure behavior, and simulation adapter; do not create a second
  proof-only execution path.

## Compare and activate

- [x] Display active-version and draft-trial final cards side by side, aligned
  by calibration source. Final cards are primary; master/art differences are
  available behind each pair.
- [x] Show a concise configuration diff grouped by Generation, References,
  Processing, and Card. Include style/checksum IDs and trial cost in details.
- [x] Never allow item-level mixing into a style version. A trial is reviewable
  and lockable only when every cohort item is ready through card assembly.
- [x] Provide **Make this the active style** only on a complete trial whose draft
...
- [x] Keep previous versions immutable and selectable for inspection. Switching
  back to an older locked version is explicit and does not rewrite approvals.
- [x] After activation, offer to create a new production batch for selected
  sources. Do not silently regenerate existing approved cards.

## Extensibility boundary

- [x] Keep Style Studio's trial, compare, lock, history, and activation flow
  independent of `amiga-ocs`.
- [x] Use driver-provided label, preview/target assets, output description,
  validation, and editor descriptors for engine-specific controls.
- [x] Confirm in a contract test that a minimal fake renderer can participate in
  draft validation and a style trial without edits to trial/approval stores.
- [x] Do not ship a user-visible “New style family” button, alternate engine,
...
- [x] Add store/API tests for draft derivation, ordered reference edits, target
...
- [x] Add UI tests for entering from the style chip, active-versus-draft clarity,
...
- [x] Add a fake-adapter integrated trial test proving the same master produces
  the same Amiga art/card in Style Studio and production.
- [x] Verify a changed dither/palette/framing value changes the draft style
  checksum and final output while leaving the active version and its cards
  byte-identical until activation.
- [x] Ensure target examples are visible but absent from every captured provider
  payload.
- [x] Run Python tests, `npm test -- --run`, `npm run build`, and
