...
- [x] Trace every frontend request and backend route used by Sources, Cards, and
...
- [x] Delete unused React components/types/tests for the six-stage workflow,
  including `FinishLab.tsx` and `SetViews.tsx` when no active imports remain.
- [x] Simplify or split `tools/portraits/workflow.py` and
  ...
  supported pipelines.
- [x] Preserve historical files under existing `portrait-library/` directories;
...
- [x] Stop seeding the old Estate reference pack in fresh workspaces. Seed the
  Amiga style/reference contract instead. Do not delete old workspace references
  or mutate old recipe snapshots.
- [x] Search active source/docs for Explore, Finish, Build Set, Frames,
...
- [x] Remove generated caches, `__pycache__`, local screenshots not selected for
  review, and temporary smoke-test workspaces from the change set.

## Documentation

- [x] Rewrite `README.md` around the three visible surfaces, startup commands,
  environment keys, explicit paid actions, active style, workspace location,
  downloads, and reset procedure.
- [x] Rewrite `docs/portrait-workbench.md` with the unified style-version,
  production-item, approval, render-revision, reference-role, and compatibility
  contracts.
- [x] Document the exact current pipeline order and make clear that the master
  is internal while the final card is the approval artifact.
- [x] Document how to add a future renderer family at the registry/contract
  level, using 8-bit or ASCII only as non-implemented examples.
- [x] Document how to regenerate the Amiga proof through the shared renderer and
  how to run the golden test. Do not document the proof CLI as a separate
  production workflow.

## Automated verification

- [x] Run the complete Python suite with `uv run --extra dev pytest -q`.
- [x] Run the complete UI suite with `npm test -- --run`.
- [x] Run `npm run build` and `git diff --check`.
- [x] From an empty temporary workspace, start the server, fetch bootstrap,
...
- [x] Run an existing-workspace smoke test proving old assets are untouched and
  legacy records do not enter current approval counts.
- [x] Confirm no automated test makes a paid external request and no credential
  value appears in logs or tracked files.

## Visual proof

- [x] Drive the real integrated app with the known stage reference and the six
...
- [x] Confirm the integrated stage-reference final art is byte-identical to
  `tools/cards/assets/amiga-ocs-portrait-v1/target-example-01.png`.
- [x] Compare the integrated card grid with
...
- [x] Review the actual browser at desktop and narrow widths. Verify the normal
  path can be explained as choose images → make cards → approve cards without
  referring to internal run stages.
- [x] Save a concise review set under `_notes/reviews/unified-card-workbench/`
  ...
  differences.
- [x] Check the review screens for button overload: advanced provenance and
  style controls begin collapsed, rare actions are secondary, and each decision
  region has one obvious next action.

## Optional live smoke

- [x] If credentials are configured and the user has separately authorized a
...
- [x] Update every checklist in Plans 00–05 to reflect verified work.
- [x] Move all completed plan files from `_plans/` to `_notes/archived/` in the
  same final change. Leave no completed plan in `_plans/`.
- [x] Confirm the final worktree contains only intended source, test, docs, and
  selected review artifacts.
- [x] Commit and push all implementation and archival changes according to
  `AGENTS.md`; confirm the branch is clean and tracks the pushed commit.
- [x] Report the active style version/checksum, final routes, test results,
