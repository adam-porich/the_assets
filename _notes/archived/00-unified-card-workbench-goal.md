...
- [x] Treat source selection and card approval as the two everyday decisions.
- [x] Make one finished candidate per selected source by default; **Try another**
  creates one additional immutable attempt for that source.
- [x] Show final cards first everywhere. Raw img2img masters are diagnostic
  artifacts, not candidates that can be approved.
- [x] Ship Amiga OCS Portrait v1 as the sole active style family and initial
  locked style version.
- [x] Keep the style pipeline extensible by renderer/assembler driver ID and
  validated configuration. Do not implement 8-bit, ASCII, ANSI, C64, or other
  style families in this plan set.
- [x] Keep target examples visually available to reviewers but technically
  impossible to send as generation references.
- [x] Apply framing before deterministic quantisation. A framing adjustment
  rerenders the existing master without making another paid generation call.
- [x] Preserve exact provenance, immutable inputs/outputs, reference ordering,
  provider usage, and cost even when those details are collapsed in the UI.
- [x] Preserve existing workspace files non-destructively. Historical Explore,
...
- [x] Require an explicit click before any paid generation. Automated tests use
  the simulation/fake adapter only.

## Target information architecture

- [x] Use only **Sources** and **Cards** as the primary navigation.
- [x] Put the active style name/version in a header chip that opens **Style
  Studio**; Style Studio is not another required production stage.
- [x] Keep run history and provenance as secondary drawers/details rather than
  primary navigation stages.
- [x] Remove the active UI language and routes for Explore, Finish, Build Set,
  Frames, candidate handoff, locked Finish anchors, and treatments.
- [x] Keep direct hashes refreshable for Sources, Cards, Style Studio, and an
  individual card. Redirect old hashes to the nearest useful new view.

## Definition of done

- [x] A fresh workspace opens with Amiga OCS Portrait v1 active and explains
  how to add/select sources.
- [x] From selected sources, one explicit action creates a production batch;
  every successful generation is automatically processed into Amiga art and a
  complete card before it appears as ready.
- [x] A reviewer can approve one current-style card per source, try another,
  adjust framing without regeneration, download approved PNGs, and see progress
  without visiting another stage.
- [x] A style editor can change generation direction/references and Amiga
  processing settings, run an integrated cohort trial, compare final cards, and
  activate only a complete trial.
- [x] The known generation reference renders byte-for-byte to the checked-in
  target example through the same production rendering path.
- [x] Existing painterly source outputs used in the proof produce a coherent
  final-card grid through the integrated application path, not a proof-only CLI.
- [x] Python tests, UI tests, TypeScript build, empty-workspace smoke test, and
  fake-adapter end-to-end workflow all pass.
- [x] Current README and docs describe only the new primary workflow.
- [x] A visual review artifact demonstrates the final Sources, Cards, and Style
  Studio screens and includes cards comparable to the approved Amiga proof.
- [x] All completed plan files are moved to `_notes/archived/`; implementation
  changes are committed and pushed, and the worktree is clean.

## Execution discipline

- [x] Read the whole plan set and current `AGENTS.md` before implementation.
- [x] Work sequentially through Plans 01–05, updating checkboxes only after the
  corresponding behavior and tests exist.
- [x] Reuse current generation, workspace, atomic-write, capability, and run
...
- [x] Commit and push coherent, passing increments. Do not commit credentials,
  paid test output, `portrait-library/`, caches, or build products.
- [x] If a detail is unspecified, choose the option that keeps the normal path
  closest to source selection → finished-card review → approval.

## Plan order

- [x] Complete **01 — Build the unified style pipeline contract**.
- [x] Complete **02 — Build card production and approval**.
- [x] Complete **03 — Replace the UI with the source-to-cards workflow**.
- [x] Complete **04 — Build Style Studio around final-card trials**.
- [x] Complete **05 — Cut over, prove the result, and archive the plans**.
