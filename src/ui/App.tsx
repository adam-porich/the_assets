...
export function App() {
  ...
  useEffect(() => { if (!message) return; const timer = window.setTimeout(() => setMessage(undefined), 4500); return () => window.clearTimeout(timer); }, [message]);
  ...
  return <main className="app-shell"><header className="app-header"><div className="brand-block"><span className="brand-symbol">✳</span><div><p className="brand-kicker">Experimental asset studio</p><h1>Portrait Workbench</h1></div></div><nav className="main-nav"><button className={route.view === "lab" || route.view === "run" || route.view === "compare" ? "active" : ""} onClick={() => navigate("#lab")}>Style Lab</button><button className={route.view === "cards" || route.view === "card" ? "active" : ""} onClick={() => navigate("#cards")}>Card drafts <span>{bootstrap.cards.length}</span></button></nav><div className="header-status">{workspace.sources.length} sources · {workspace.references.length} references</div></header><div className="global-band"><span>Workbench state</span><span className="global-rule" />{message && <span className={`toast-inline ${message.kind}`}>{message.text}</span>}<span className="global-spacer" /><button className="text-button" onClick={() => void refresh()}>Refresh</button></div>{route.view === "lab" || route.view === "run" ? <StyleLab workspace={workspace} models={bootstrap.models} runs={bootstrap.runs} activeRun={activeRun} onWorkspace={workspaceUpdate} onRefresh={refresh} onNavigate={navigate} onMessage={notify} /> : route.view === "compare" && route.id && route.other ? <CompareView firstId={route.id} secondId={route.other} onNavigate={navigate} /> : route.view === "cards" ? <CardGallery cards={bootstrap.cards} onNavigate={navigate} onRefresh={refresh} /> : route.view === "card" && card ? <CardWorkbench card={card} workspace={workspace} onNavigate={navigate} onMessage={notify} onRefresh={refresh} /> : <div className="empty-panel large-empty"><h2>Card draft not found</h2><button className="button primary" onClick={() => navigate("#cards")}>Open card drafts</button></div>}<footer className="app-footer"><span>Painterly benchmark experiments · not production approval tooling</span><span>Sources and references stay in portrait-library/</span></footer></main>;
}
