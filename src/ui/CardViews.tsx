...
export function FramesStage({ cards, initialCard, onNavigate, onMessage, onRefresh, activeSetId, setItemCount }: StageProps & { initialCard?: Card }) {
  ...
  if (!card) return <section className="stage-page frames-page"><div className="page-heading"><div><p className="eyebrow">Stage 5 of 6 · Frames</p><h2>Frame active-set portraits</h2><p>{activeSetId ? `The active set supplies every card. Progress: ${working.length} working · ${cards.filter((item) => item.decision === "keep").length} kept · ${cards.filter((item) => item.decision === "discard").length} discarded${setItemCount ? ` of ${setItemCount}` : ""}.` : "Build a ready set first; Frames creates one draft for each set item automatically."}</p></div><button className="button secondary" onClick={() => onNavigate(activeSetId ? "#set" : "#styles")}>← Back to {activeSetId ? "Build Set" : "Explore"}</button></div>{working.length ? <div className="candidate-grid">{working.map((item) => <article className="candidate-card" key={item.card_id}><img src={`${item.render_url}?v=${encodeURIComponent(item.updated_at)}`} alt={item.label} /><div><strong>{summary(item, "source_label") || item.label}</strong><small>{item.set_id ? "Active set portrait" : summary(item, "change_note") || "Legacy card"}</small><button className="button primary" onClick={() => onNavigate(`#frames/${item.card_id}`)}>Choose frame →</button></div></article>)}</div> : <div className="empty-panel"><span className="empty-glyph">▱</span><h3>{activeSetId ? "No working portraits" : "No active set"}</h3><p>{activeSetId ? "Every ready set item is drafted automatically when you enter Frames." : "Complete Build Set first. Historical and legacy cards remain available without entering the active Completed set."}</p><button className="button primary" onClick={() => onNavigate(activeSetId ? "#set" : "#styles")}>Open {activeSetId ? "Build Set" : "Explore"}</button></div>}<div className="stage-actions"><button className="button secondary" onClick={() => onNavigate(activeSetId ? "#set" : "#styles")}>← Back</button><button className="button primary" onClick={() => onNavigate("#completed")}>View Completed →</button></div></section>;
  ...
  return <section className="stage-page frames-page"><div className="page-heading"><div><p className="eyebrow">Stage 5 of 6 · Frames</p><h2>Choose the card frame</h2><p>These are exact saved renders, not approximations. Pick a composition, then its Treatment.</p></div><button className="button secondary" onClick={() => onNavigate("#frames")}>← Candidate list</button></div>
...
export function CompletedStage({ cards, onNavigate, onMessage, onRefresh, activeSetId, legacyCards = [] }: StageProps & { legacyCards?: Card[] }) {
  ...
  return <section className="stage-page completed-page"><div className="page-heading"><div><p className="eyebrow">Stage 6 of 6 · Completed</p><h2>{activeSetId ? "Completed active set" : "Completed cards"}</h2><p>{activeSetId ? "Only kept cards from the active set appear here. Every card traces to the same locked Finish." : "Legacy cards remain inspectable, but are not mixed into a coherent active set."}</p></div><button className="button secondary" onClick={() => onNavigate("#frames")}>← Back to Frames</button></div>{kept.length ? <div className="completed-grid">{kept.map((card) => <article className="completed-card surface" key={card.card_id}><img src={`${card.render_url}?v=${encodeURIComponent(card.updated_at)}`} alt={card.label} /><div><p className="eyebrow">Completed · {activeSetId ? "Active set" : "Legacy"}</p><h3>{summary(card, "source_label") || card.label}</h3><p>{summary(card, "change_note") || "Structured art direction"}</p><span>{card.finish_id ? `Finish ${card.finish_id}` : summary(card, "recipe_name") || card.run_id} · {card.archetype} · Treatment: {card.treatment === "estate-pixel-v1" ? "Estate Pixel" : "Painterly"}</span><div><a className="button primary" href={card.render_url} download={`${card.label.replace(/[^a-z0-9]+/gi, "-").toLowerCase() || card.card_id}.png`}>Download PNG</a><button className="button secondary" onClick={() => void reconsider(card)}>Reconsider in Frames</button></div></div></article>)}</div> : <div className="empty-panel"><span className="empty-glyph">✓</span><h3>{activeSetId ? "No kept cards in this set" : "Nothing completed yet"}</h3><p>{activeSetId ? "Keep each framed active-set portrait to complete the coherent set." : "Keep a framed candidate and its full render will appear here."}</p><button className="button primary" onClick={() => onNavigate("#frames")}>Open Frames</button></div>}{activeSetId && legacyCards.length > 0 && <details className="surface legacy-history"><summary>Legacy cards ({legacyCards.length})</summary><div className="legacy-card-list">{legacyCards.map((card) => <article key={card.card_id}><img src={card.render_url} alt={card.label} /><span><strong>{card.label}</strong><small>Pre-set card · {card.decision}</small></span><button className="button secondary" onClick={() => onNavigate(`#frames/${card.card_id}`)}>Open history</button></article>)}</div></details>}</section>;
}
