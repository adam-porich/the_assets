...
export function SourcesView({ bootstrap, navigate, refresh, notify }: Shared) {
  ...
  return <section className="page sources-page">
    ...
    <section className="surface source-management">
      ...
      {results.length > 0 && <div className="search-results">{results.map((result) => <article key={result.pexels_photo_id}><img src={result.preview_url || result.selected_image_url} alt={result.label || result.photographer || "Search result"} /><div><strong>{result.photographer || "Image source"}</strong><button type="button" className="button secondary" onClick={() => void api.importSources([result]).then(() => refresh()).then(() => notify("Source added to Candidates")).catch((error) => notify((error as Error).message, "error"))}>Add source</button></div></article>)}</div>}
    </section>
    ...
    <section className="surface pipeline-launcher">
      ...
      <div className="launcher-action"><p>{selected.length * 2} generation calls · {modelCost(model)} each. Every source is normalised, then stylised.</p><button className="button primary large" disabled={Boolean(problem) || Boolean(busy)} onClick={() => void runPipeline()}>{busy === "run" ? "Starting pipeline…" : `Run ${selected.length} source${selected.length === 1 ? "" : "s"}`}</button>{problem && <p className="validation-note" role="status">{problem}</p>}</div>
    </section>
  </section>;
}
...
function PipelineDiagram({ style, cards, sources }: { style?: PipelineStyle; cards: ProducedCard[]; sources: Source[] }) {
  ...
  return <section className="surface pipeline-diagram">
    <div className="diagram-heading"><div><p className="eyebrow">Pipeline anatomy</p><h3>Source in. Card out.</h3></div><span>{cards.length} produced result{cards.length === 1 ? "" : "s"}</span></div>
    <div className="diagram-track">
      <figure>{sourceUrl ? <img src={sourceUrl} alt="Source input" /> : <span className="diagram-empty">＋</span>}<figcaption><small>Input</small><strong>Source content</strong></figcaption></figure>
      <div className="diagram-step"><span>→</span><small>{model}</small></div>
      <figure>{example?.normalised_url ? <img src={example.normalised_url} alt="Normalised source" /> : <span className="diagram-empty">clean</span>}<figcaption><small>Stage 1</small><strong>Normalised</strong></figcaption></figure>
      ...
      <div className="diagram-step"><span>→</span><small>{model}</small></div>
      <figure>{example?.master_url ? <img src={example.master_url} alt="Styled master" /> : <span className="diagram-empty">master</span>}<figcaption><small>Stage 2</small><strong>Styled master</strong></figcaption></figure>
...
export function PipelinesView({ bootstrap, navigate, refresh, notify, pipelineId }: Shared & { pipelineId?: string }) {
  ...
  return <section className="page pipelines-page">
    <div className="page-heading"><div><p className="eyebrow">Pipeline</p><h2>Normalise, then stylise</h2><p>One universal workflow preserves the source first, then applies the Amiga visual language.</p></div><button className="button secondary" onClick={() => navigate("#candidates")}>Open candidates</button></div>
    ...
    {draft && draftPipelineId === selectedId && <>
      ...
      <section className="surface editor"><div className="panel-heading"><div><p className="eyebrow">Transform</p><h3>Edit working pipeline</h3></div><span className="status-chip working">unsaved version</span></div>
        ...
        {(["normalise", "stylise"] as const).map((stage) => <details key={stage} open><summary>{stage === "normalise" ? "1 · Normalise prompt" : "2 · Stylise prompt"}</summary><textarea value={style.generation.stages?.[stage].prompt || ""} onChange={(event) => setStyle({ ...style, generation: { ...style.generation, stages: { normalise: { prompt: style.generation.stages?.normalise.prompt || "" }, stylise: { prompt: style.generation.stages?.stylise.prompt || "" }, [stage]: { prompt: event.target.value } } } })} onBlur={() => void patchGeneration({ stages: style.generation.stages })} /></details>)}
...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  return <section className="page candidates-page">
    <div className="page-heading"><div><p className="eyebrow">Candidate packs</p><h2>Next. Next. Ooh, a good one.</h2><p>Each source keeps its three newest cards in view. Generate runs normalise then stylise, and the keepers go to Collection.</p></div><span className="status-chip">3 cards per source</span></div>
    {!sources.length ? <div className="empty-panel surface"><span className="empty-glyph">→</span><h3>Choose at least one source first</h3><button className="button primary" onClick={() => navigate("#sources")}>Choose sources</button></div> : <div className="candidate-packs">{sources.map((source) => {
