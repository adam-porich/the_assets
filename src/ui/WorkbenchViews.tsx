...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  return <section className="page candidates-page">
    ...
    {selectedCard && <section className="surface result-inspector">
      <div className="panel-heading"><div><p className="eyebrow">{selectedCard.pipeline_label} · {selectedCard.source_label}</p><h3>Candidate details</h3><p>Inspect the foreground and composite, then adjust placement, background, or palette without regenerating.</p></div><button className="button secondary" onClick={() => navigate("#candidates")}>Close</button></div>
      <div className="asset-stages"><figure><img src={selectedCard.source_url} alt="Prepared Input" /><figcaption>Input</figcaption></figure><span>→</span>{selectedCard.foreground_url && <><figure><img src={selectedCard.foreground_url} alt="Isolated foreground" /><figcaption>Foreground</figcaption></figure><span>→</span></>}<figure><img src={selectedCard.master_url} alt="Background composite" /><figcaption>Composite</figcaption></figure><span>→</span><figure><img className="pixelated" src={selectedCard.art_url} alt="Amiga art" /><figcaption>Rendered art</figcaption></figure><span>→</span><figure className="final-stage"><img className="pixelated" src={selectedCard.card_url} alt="Final card" /><figcaption>Candidate card</figcaption></figure></div>
      <div className="framing-controls"><label>Zoom<input type="range" min="1" max="3" step="0.01" value={framing.zoom} onChange={(event) => setFraming({ ...framing, zoom: Number(event.target.value) })} /></label><label>Horizontal<input type="range" min="-1" max="1" step="0.01" value={framing.offset_x} onChange={(event) => setFraming({ ...framing, offset_x: Number(event.target.value) })} /></label><label>Vertical<input type="range" min="-1" max="1" step="0.01" value={framing.offset_y} onChange={(event) => setFraming({ ...framing, offset_y: Number(event.target.value) })} /></label>{selectedCard.foreground_url && <label>Background<select value={backgroundId} onChange={(event) => setBackgroundId(event.target.value)}>{backgroundConfig(bootstrap.style.active).presets.map((background) => <option key={background.id} value={background.id}>{background.label}</option>)}</select></label>}<label>Palette<select value={paletteMode} onChange={(event) => setPaletteMode(event.target.value as "fixed-house" | "adaptive-hybrid")}><option value="adaptive-hybrid">Adaptive · 10 anchors + 22 image colours</option><option value="fixed-house">Fixed · original house 32</option></select></label></div>
