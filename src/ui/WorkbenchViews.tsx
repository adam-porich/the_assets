...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  const activeBatch = bootstrap.batches.find((batch) => isActive(batch.status));
  useEffect(() => { if (selectedCard) { setFraming(selectedCard.framing || { zoom: 1, offset_x: 0, offset_y: 0 }); setPaletteMode(selectedCard.palette_mode || (selectedCard.render_metadata?.palette_mode === "adaptive-hybrid" ? "adaptive-hybrid" : "fixed-house")); setBackgroundId(selectedCard.background_id || backgroundConfig(bootstrap.style.active).default_id); } }, [selectedCard?.item_id]);
  useEffect(() => {
  ...
  async function tryAnother(card: ProducedCard) {
    ...
    if (source) openGeneration(source, card.pipeline_id, card.content_direction, card.background_id);
  }
  ...
  async function render(card: ProducedCard) {
    ...
    try { await api.renderFraming(card.batch_id, card.item_id, framing, paletteMode, backgroundId); notify("Composition, background, and palette rerendered"); await refresh(); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  ...
  return <section className="page candidates-page">
    ...
    {selectedCard && <div className="dialog-backdrop" role="presentation"><section className="surface input-dialog candidate-dialog result-inspector" role="dialog" aria-modal="true" aria-labelledby="candidate-details-title">
      ...
      <div className="candidate-preview foreground-flow"><figure>{selectedCard.source_url ? <img src={selectedCard.source_url} alt="Prepared Input" /> : <div className="preview-placeholder">Input unavailable</div>}<figcaption>Input</figcaption></figure><span>→</span><figure>{selectedCard.foreground_url ? <img src={selectedCard.foreground_url} alt="Isolated foreground" /> : <div className="preview-placeholder">Foreground unavailable</div>}<figcaption>Foreground</figcaption></figure><span>→</span><figure>{selectedCard.master_url ? <img src={selectedCard.master_url} alt="Background composite" /> : <div className="preview-placeholder">Composite unavailable</div>}<figcaption>Background composite</figcaption></figure><span>→</span><figure className="candidate-preview-card">{selectedCard.card_url ? <img className="pixelated" src={selectedCard.card_url} alt="Candidate card" /> : <div className="preview-placeholder">Card unavailable</div>}<figcaption>Candidate card</figcaption></figure></div>
      <div className="framing-controls"><label>Zoom<input type="range" min="1" max="3" step="0.01" value={framing.zoom} onChange={(event) => setFraming({ ...framing, zoom: Number(event.target.value) })} /></label><label>Horizontal<input type="range" min="-1" max="1" step="0.01" value={framing.offset_x} onChange={(event) => setFraming({ ...framing, offset_x: Number(event.target.value) })} /></label><label>Vertical<input type="range" min="-1" max="1" step="0.01" value={framing.offset_y} onChange={(event) => setFraming({ ...framing, offset_y: Number(event.target.value) })} /></label>{selectedCard.foreground_url && <label>Background<select value={backgroundId} onChange={(event) => setBackgroundId(event.target.value)}>{backgroundConfig(selectedCardStyle).presets.map((background) => <option key={background.id} value={background.id}>{background.label}</option>)}</select></label>}<label>Palette<select value={paletteMode} onChange={(event) => setPaletteMode(event.target.value as "fixed-house" | "adaptive-hybrid")}><option value="adaptive-hybrid">Adaptive · 10 anchors + 22 image colours</option><option value="fixed-house">Fixed · original house 32</option></select></label></div>
      <div className="inspector-actions"><button className="button primary" disabled={Boolean(busy)} onClick={() => void toggleFavorite(selectedCard)}>{selectedCard.favorite ? "★ Saved to Collection" : "☆ Save to Collection"}</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void tryAnother(selectedCard)}>Generate another result</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void render(selectedCard)}>{busy === `render-${selectedCard.item_id}` ? "Rerendering…" : "Save composite · no generation"}</button></div><ResultProvenance card={selectedCard} />
    </section></div>}
  </section>;
}
