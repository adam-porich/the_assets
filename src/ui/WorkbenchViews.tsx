...
function Provenance({ item, batch }: { item: ProductionItem; batch: ProductionBatch }) {
  ...
  return <details><summary>How this was made</summary><div className="details-content"><p>Source identity: {item.source_url ? <a href={item.source_url}>original source</a> : "—"} · canonical master: {item.master_url ? <a href={item.master_url}>inspect master</a> : "—"}</p><p>Amiga art: {item.art_url ? <a href={item.art_url}>336×276 PNG</a> : "—"} · card: {item.card_url ? <a href={item.card_url}>420×600 PNG</a> : "—"}</p><p>Generation: <strong>{String(item.generation?.execution_mode || batch.model_capabilities?.execution_mode || "unknown")}</strong> · {String(item.generation?.model || batch.model?.name || batch.style_version_id)} · {refs.length} ordered style reference{refs.length === 1 ? "" : "s"}</p><p>Style checksum: <code>{batch.style_checksum_sha256}</code> · render revision {item.render_revision} · card checksum <code>{item.card_checksum_sha256 || "—"}</code></p><p>The target example is review-only and was excluded from the provider request.</p></div></details>;
}
...
export function CardsView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  const selectedBatch = bootstrap.batches.find((candidate) => candidate.batch_id === batchId) || bootstrap.batches.find((candidate) => candidate.purpose === "card-production");
