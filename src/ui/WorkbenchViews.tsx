...
import { api } from "./api";
import type { Bootstrap, Framing, InputAsset, Model, PipelineRecord, PipelineStyle, ProducedCard, ProductionBatch, ProductionItem, SearchResult } from "./types";
...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  const activeBatch = bootstrap.batches.find((batch) => isActive(batch.status));
  useEffect(() => { if (selectedCard) { setFraming(selectedCard.framing || { zoom: 1, offset_x: 0, offset_y: 0 }); setPaletteMode(selectedCard.palette_mode || (selectedCard.render_metadata?.palette_mode === "adaptive-hybrid" ? "adaptive-hybrid" : "fixed-house")); setBackgroundId(selectedCard.background_id || backgroundConfig(bootstrap.style.active).default_id); setRenderPreview(undefined); } }, [selectedCard?.item_id]);
  useEffect(() => {
  ...
  async function tryAnother(card: ProducedCard) {
    ...
    if (source) { navigate("#candidates"); openGeneration(source, card.pipeline_id, card.content_direction, card.background_id); }
  }
  ...
  async function render(card: ProducedCard) {
    ...
    try { await api.renderFraming(card.batch_id, card.item_id, framing, paletteMode, backgroundId); setRenderPreview(undefined); notify("Composition, background, and palette saved"); await refresh(); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  async function previewRender(card: ProducedCard) {
    setBusy(`preview-${card.item_id}`);
    try { setRenderPreview((await api.previewFraming(card.batch_id, card.item_id, framing, paletteMode, backgroundId)).preview); notify("Preview updated · candidate not saved"); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  ...
  return <section className="page candidates-page">
    ...
    {selectedCard && !generationSource && <div className="dialog-backdrop" role="presentation"><section className="surface input-dialog candidate-dialog result-inspector" role="dialog" aria-modal="true" aria-labelledby="candidate-details-title">
      <div className="panel-heading"><div><p className="eyebrow">{selectedCard.pipeline_label} · {selectedCard.source_label}</p><h3 id="candidate-details-title">Candidate details</h3><p>Inspect the foreground and composite, then adjust placement, background, or palette without regenerating.</p></div><button className="button secondary" onClick={() => navigate("#candidates")}>Close</button></div>
      ...
      <div className="framing-controls"><label>Zoom<input type="range" min="1" max="3" step="0.01" value={framing.zoom} onChange={(event) => { setFraming({ ...framing, zoom: Number(event.target.value) }); setRenderPreview(undefined); }} /></label><label>Horizontal<input type="range" min="-1" max="1" step="0.01" value={framing.offset_x} onChange={(event) => { setFraming({ ...framing, offset_x: Number(event.target.value) }); setRenderPreview(undefined); }} /></label><label>Vertical<input type="range" min="-1" max="1" step="0.01" value={framing.offset_y} onChange={(event) => { setFraming({ ...framing, offset_y: Number(event.target.value) }); setRenderPreview(undefined); }} /></label>{selectedCard.foreground_url && <label>Background<select value={backgroundId} onChange={(event) => { setBackgroundId(event.target.value); setRenderPreview(undefined); }}>{backgroundConfig(selectedCardStyle).presets.map((background) => <option key={background.id} value={background.id}>{background.label}</option>)}</select></label>}<label>Palette<select value={paletteMode} onChange={(event) => { setPaletteMode(event.target.value as "fixed-house" | "adaptive-hybrid"); setRenderPreview(undefined); }}><option value="adaptive-hybrid">Adaptive · 10 anchors + 22 image colours</option><option value="fixed-house">Fixed · original house 32</option></select></label></div>
      <div className="inspector-actions"><button className="button secondary" disabled={Boolean(busy) || Boolean(renderPreview)} title={renderPreview ? "Save the preview changes before adding this candidate to Collection" : undefined} onClick={() => void toggleFavorite(selectedCard)}>{selectedCard.favorite ? "★ Saved to Collection" : "☆ Save to Collection"}</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void tryAnother(selectedCard)}>Generate another result</button><button className="button primary" disabled={Boolean(busy)} onClick={() => void previewRender(selectedCard)}>{busy === `preview-${selectedCard.item_id}` ? "Previewing…" : "Preview changes"}</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void render(selectedCard)}>{busy === `render-${selectedCard.item_id}` ? "Saving…" : "Save changes"}</button></div><ResultProvenance card={selectedCard} />
    </section></div>}
  </section>;
}
