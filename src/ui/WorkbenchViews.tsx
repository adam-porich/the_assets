...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  const [busy, setBusy] = useState("");
  const [choosingSource, setChoosingSource] = useState("");
  ...
  async function generate(sourceIds: string[], selectedId: string) {
    ...
    try { await api.createProduction(sourceIds, selectedId, pipeline?.style.generation.execution_mode === "live"); setChoosingSource(""); notify(`New ${pipeline?.label || "pipeline"} candidate started`); await refresh(); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  ...
  return <section className="page candidates-page">
    <div className="page-heading"><div><p className="eyebrow">Candidate packs</p><h2>Next. Next. Ooh, a good one.</h2><p>Each source keeps its three newest cards in view. Generate another with either pipeline, then favorite the keepers.</p></div><span className="status-chip">3 cards per source</span></div>
    ...
    {!sources.length ? <div className="empty-panel surface"><span className="empty-glyph">→</span><h3>Choose at least one source first</h3><button className="button primary" onClick={() => navigate("#sources")}>Choose sources</button></div> : <div className="candidate-packs">{sources.map((source) => {
      ...
      return <section className="surface candidate-pack" key={source.id}>
        <div className="pack-cards">{cards.length ? cards.map((card, index) => <div className={`generation-card ${selectedCard?.item_id === card.item_id ? "selected" : ""}`} key={`${card.batch_id}:${card.item_id}`}><button className="candidate-open" onClick={() => navigate(`#candidates/${card.batch_id}/${card.item_id}`)}><img src={card.card_url} alt={`${source.label} ${card.pipeline_label} candidate ${index + 1}`} /><span><strong>{index === 0 ? "Newest card" : `Previous ${index}`}</strong><small>{card.pipeline_label}</small><small>{generationOrigin(card)}</small></span></button><button className={`favorite-button ${card.favorite ? "saved" : ""}`} aria-label={card.favorite ? "Remove from Collection" : "Save to Collection"} disabled={Boolean(busy)} onClick={() => void toggleFavorite(card)}>{card.favorite ? "★" : "☆"}</button></div>) : <div className="pack-empty"><span>◇</span><strong>No cards yet</strong><small>Open your first one.</small></div>}</div>
        <aside className="pack-generate">
          ...
          <div className={`pack-action ${choosingSource === source.id ? "choosing" : ""}`}>{choosingSource === source.id ? <><strong>Choose a pipeline</strong>{pipelines.map((pipeline) => { const model = bootstrap.models.find((entry) => entry.id === pipeline.style.generation.model_id && entry.execution_mode === pipeline.style.generation.execution_mode); return <button className="pipeline-choice" key={pipeline.pipeline_id} disabled={Boolean(activeBatch) || Boolean(busy) || !model?.available || !model.credentials_configured} onClick={() => void generate([source.id], pipeline.pipeline_id)}><span>{pipeline.label}</span><small>{pipeline.active ? "active default · " : ""}{modelCost(model)}</small></button>; })}<button className="text-button" onClick={() => setChoosingSource("")}>Cancel</button></> : <button className="generate-card" disabled={Boolean(activeBatch) || Boolean(busy)} onClick={() => setChoosingSource(source.id)}><span>＋</span><strong>Generate new</strong><small>Choose pipeline</small></button>}</div>
        </aside>
      </section>;
    })}</div>}
