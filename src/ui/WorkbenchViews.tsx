...
export function InputsView({ bootstrap, refresh, notify }: Shared) {
  ...
  const [normalisationMode, setNormalisationMode] = useState<"preserve" | "reconstruct">(bootstrap.normalisation.default_mode || "preserve");
  ...
  function open(item: InputAsset) { setEditing(item); setPrompt(item.accepted_normalisation?.prompt || bootstrap.normalisation.default_prompt); setQuality(item.accepted_normalisation?.quality || bootstrap.normalisation.default_quality); setNormalisationMode(item.accepted_normalisation?.mode || bootstrap.normalisation.default_mode || "preserve"); }
  ...
  async function normalise() {
    if (!editing) return;
    setBusy("normalise");
    try { setEditing((await api.normaliseInput(editing.id, prompt, quality, normalisationMode === "reconstruct" && pipeline.generation.execution_mode === "live", normalisationMode)).input); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  ...
  return <section className="page inputs-page">
    ...
    {editing && <div className="dialog-backdrop" role="presentation"><section className="surface input-dialog" role="dialog" aria-modal="true" aria-labelledby="input-dialog-title"><div className="panel-heading"><div><p className="eyebrow">Prepare Input</p><h3 id="input-dialog-title">{editing.label}</h3></div><button className="button secondary" disabled={isActive(latest?.status || "") || busy === "accept"} onClick={() => void close()}>Cancel</button></div><div className="normalisation-preview"><figure>{editing.original_url && <img src={editing.original_url} alt="Original" />}<figcaption>Original</figcaption></figure><span>→</span><figure>{latest?.preview_url ? <img src={latest.preview_url} alt="Normalized preview" /> : editing.image_url ? <img src={editing.image_url} alt="Accepted Input" /> : <div className="preview-placeholder">{isActive(latest?.status || "") ? <span className="spinner" /> : "Preview"}</div>}<figcaption>{latest?.status === "failed" ? "Preview failed" : "Prepared Input"}</figcaption></figure></div>{latest?.error && <p className="validation-note" role="alert">{latest.error}</p>}<label className="prompt-field">Preparation mode<select value={normalisationMode} onChange={(event) => setNormalisationMode(event.target.value as typeof normalisationMode)}><option value="preserve">Preserve · crop and pad only</option><option value="reconstruct">Reconstruct · generative cleanup</option></select></label>{normalisationMode === "reconstruct" && <label className="prompt-field">Reconstruction prompt<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} /></label>}<div className="dialog-controls">{normalisationMode === "reconstruct" ? <label>Quality<select value={quality} onChange={(event) => setQuality(event.target.value as typeof quality)}>{(["low", "medium", "high"] as const).map((value) => <option key={value} value={value} disabled={Boolean(model?.qualities.length) && !model?.qualities.includes(value)}>{value}</option>)}</select></label> : <small>Free · no generative redraw</small>}<div><small>{normalisationMode === "reconstruct" ? `${model?.name || pipeline.generation.model_id} · ${modelCost(model)} per preview` : "No image generation"}</small><button className="button secondary" disabled={(normalisationMode === "reconstruct" && !model?.available) || isActive(latest?.status || "") || Boolean(busy)} onClick={() => void normalise()}>{isActive(latest?.status || "") ? "Preparing…" : latest?.status === "ready" ? "Rerun preparation" : "Prepare preview"}</button><button className="button primary" disabled={latest?.status !== "ready" || Boolean(busy)} onClick={() => void accept()}>{busy === "accept" ? "Adding…" : "OK · Add to Inputs"}</button></div></div></section></div>}
  </section>;
}
...
function ResultProvenance({ card }: { card: ProducedCard }) {
  ...
  return <details><summary>Run details</summary><div className="details-content">{card.source_original_url && <p>Source trail: <a href={card.source_original_url} target="_blank" rel="noreferrer">original</a> → prepared Input → styled master → OCS render</p>}<p>Pipeline <code>{card.style_version_id}</code> · configuration <code>{shortChecksum(card.style_checksum_sha256)}</code></p><p>Generation: <strong>{String(card.generation?.execution_mode || "unknown")}</strong> · {String(card.generation?.model || "model unavailable")}</p>{card.content_direction && <p>Content direction: {card.content_direction}</p>}{card.prompt_override && <p>Legacy style override: {card.prompt_override}</p>}<p>Ordered references: {refs.length ? refs.map((reference, index) => <span key={String(reference.reference_id || index)}> {index ? "→ " : ""}{String(reference.label || reference.reference_id)}</span>) : "—"}</p><p>Render revision {card.render_revision} · card checksum <code>{card.card_checksum_sha256 || "—"}</code></p></div></details>;
}
...
export function CandidatesView({ bootstrap, navigate, refresh, notify, batchId, cardId }: Shared & { batchId?: string; cardId?: string }) {
  ...
  const activeBatch = bootstrap.batches.find((batch) => isActive(batch.status));
  useEffect(() => { if (selectedCard) { setFraming(selectedCard.framing || { zoom: 1, offset_x: 0, offset_y: 0 }); setPaletteMode(selectedCard.palette_mode || (selectedCard.render_metadata?.palette_mode === "adaptive-hybrid" ? "adaptive-hybrid" : "fixed-house")); } }, [selectedCard?.item_id]);
  useEffect(() => {
  ...
  function openGeneration(source: InputAsset, pipelineId = bootstrap.style.active_pipeline_id, direction?: string | null) {
    const pipeline = pipelines.find((item) => item.pipeline_id === pipelineId) || pipelines[0];
    setGenerationSource(source); setGenerationPipelineId(pipeline?.pipeline_id || ""); setContentDirection(direction || ""); setPreviewBatch(undefined);
  }
  ...
  async function generate() {
    ...
    try { const response = await api.createProduction([generationSource.id], generationPipelineId, pipeline?.style.generation.execution_mode === "live", contentDirection); setPreviewBatch(response.batch); notify("Candidate preview started"); await refresh(); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  async function tryAnother(card: ProducedCard) {
    const source = sources.find((item) => item.id === card.source_id);
    if (source) openGeneration(source, card.pipeline_id, card.content_direction);
  }
  ...
  async function render(card: ProducedCard) {
    ...
    try { await api.renderFraming(card.batch_id, card.item_id, framing, paletteMode); notify("Framing and palette rerendered"); await refresh(); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(""); }
  }
  ...
  return <section className="page candidates-page">
    ...
    {generationSource && <div className="dialog-backdrop" role="presentation"><section className="surface input-dialog candidate-dialog" role="dialog" aria-modal="true" aria-labelledby="candidate-dialog-title"><div className="panel-heading"><div><p className="eyebrow">Generate candidate</p><h3 id="candidate-dialog-title">{generationSource.label}</h3></div><button className="button secondary" disabled={isActive(previewBatch?.status || "")} onClick={() => { setGenerationSource(undefined); setPreviewBatch(undefined); }}>Cancel</button></div><div className="candidate-preview"><figure>{generationSource.image_url && <img src={generationSource.image_url} alt="Prepared Input" />}<figcaption>Input</figcaption></figure><span>→</span><figure>{previewItem?.master_url ? <img src={previewItem.master_url} alt="Styled preview" /> : <div className="preview-placeholder">{isActive(previewBatch?.status || "") ? <span className="spinner" /> : "Styled preview"}</div>}<figcaption>Styled master</figcaption></figure><span>→</span><figure className="candidate-preview-card">{previewItem?.card_url ? <img className="pixelated" src={previewItem.card_url} alt="Candidate preview" /> : <div className="preview-placeholder">Card</div>}<figcaption>Candidate card</figcaption></figure></div>{previewItem?.error && <p className="validation-note" role="alert">{previewItem.error}</p>}<label className="prompt-field">Pipeline<select value={generationPipelineId} disabled={Boolean(previewBatch)} onChange={(event) => setGenerationPipelineId(event.target.value)}>{pipelines.map((pipeline) => <option key={pipeline.pipeline_id} value={pipeline.pipeline_id}>{pipeline.label}</option>)}</select></label><label className="prompt-field">Content direction <small>optional</small><textarea value={contentDirection} placeholder="For example: make the smile unsettling, keep everything else unchanged." onChange={(event) => setContentDirection(event.target.value)} /></label><p className="validation-note">The pipeline style stays locked; this field only describes what should change in the subject or scene.</p><div className="candidate-dialog-actions"><small>{pipelines.find((item) => item.pipeline_id === generationPipelineId)?.style.generation.quality || "low"} quality · {modelCost(bootstrap.models.find((model) => model.id === pipelines.find((item) => item.pipeline_id === generationPipelineId)?.style.generation.model_id))} per preview</small><button className="button secondary" disabled={isActive(previewBatch?.status || "") || Boolean(busy) || Boolean(activeBatch && activeBatch.batch_id !== previewBatch?.batch_id)} onClick={() => void generate()}>{isActive(previewBatch?.status || "") ? "Generating…" : previewItem?.status === "ready" ? "Rerun with this direction" : "Generate preview"}</button><button className="button primary" disabled={previewItem?.status !== "ready" || Boolean(busy)} onClick={() => void acceptPreview()}>{busy === "accept-preview" ? "Accepting…" : "OK · Add to pack"}</button></div></section></div>}
    {selectedCard && <section className="surface result-inspector"><div className="panel-heading"><div><p className="eyebrow">{selectedCard.pipeline_label} · {selectedCard.source_label}</p><h3>Candidate details</h3><p>Inspect the style and render stages, adjust framing, palette, or save this candidate to Collection.</p></div><button className="button secondary" onClick={() => navigate("#candidates")}>Close</button></div><div className="asset-stages"><figure><img src={selectedCard.source_url} alt="Prepared Input" /><figcaption>Input</figcaption></figure><span>→</span><figure><img src={selectedCard.master_url} alt="Styled master" /><figcaption>Styled master</figcaption></figure><span>→</span><figure><img className="pixelated" src={selectedCard.art_url} alt="Amiga art" /><figcaption>Rendered art</figcaption></figure><span>→</span><figure className="final-stage"><img className="pixelated" src={selectedCard.card_url} alt="Final card" /><figcaption>Candidate card</figcaption></figure></div><div className="framing-controls"><label>Zoom<input type="range" min="1" max="3" step="0.01" value={framing.zoom} onChange={(event) => setFraming({ ...framing, zoom: Number(event.target.value) })} /></label><label>Horizontal<input type="range" min="-1" max="1" step="0.01" value={framing.offset_x} onChange={(event) => setFraming({ ...framing, offset_x: Number(event.target.value) })} /></label><label>Vertical<input type="range" min="-1" max="1" step="0.01" value={framing.offset_y} onChange={(event) => setFraming({ ...framing, offset_y: Number(event.target.value) })} /></label><label>Palette<select value={paletteMode} onChange={(event) => setPaletteMode(event.target.value as "fixed-house" | "adaptive-hybrid")}><option value="adaptive-hybrid">Adaptive · 10 anchors + 22 image colours</option><option value="fixed-house">Fixed · original house 32</option></select></label></div><div className="inspector-actions"><button className="button primary" disabled={Boolean(busy)} onClick={() => void toggleFavorite(selectedCard)}>{selectedCard.favorite ? "★ Saved to Collection" : "☆ Save to Collection"}</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void tryAnother(selectedCard)}>Generate another result</button><button className="button secondary" disabled={Boolean(busy)} onClick={() => void render(selectedCard)}>{busy === `render-${selectedCard.item_id}` ? "Rerendering…" : "Save render · no generation"}</button></div><ResultProvenance card={selectedCard} /></section>}
  </section>;
}
