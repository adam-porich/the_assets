...
export const api = {
  ...
  createProduction: (source_ids: string[], style_version_id: string) => request<{ batch: ProductionBatch }>("/production", json("POST", { source_ids, style_version_id })),
  ...
  retryFailed: (id: string) => request<{ batch: ProductionBatch }>(`/production/${id}/retry`, json("POST", {})),
  tryAnother: (id: string, source_id: string) => request<{ batch: ProductionBatch }>(`/production/${id}/try-another`, json("POST", { source_id })),
  ...
  runTrial: (source_ids: string[]) => request<{ batch: ProductionBatch }>("/styles/trials", json("POST", { source_ids })),
